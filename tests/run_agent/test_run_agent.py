...
class TestRunConversation:
    ...
    def test_kanban_continuation_enqueued_on_iteration_exhaustion(self, agent, monkeypatch):
        """Iteration exhaustion is a durable yield, not a failure or terminal state."""
        self._setup_agent(agent)
        ...
        mock_yield = MagicMock(return_value=SimpleNamespace(
            continuation_number=1, checkpoint_sha256="a" * 64,
        ))
        mock_connect = MagicMock(return_value=MagicMock())

        with (
            patch("run_agent.handle_function_call", return_value="ok"),
            patch("hermes_cli.kanban_db.get_task", return_value=SimpleNamespace(current_run_id=7)),
            patch("hermes_cli.kanban_db.get_saved_phase_checkpoint", return_value={"work_complete": False}),
            patch("hermes_cli.kanban_db.yield_task_for_continuation", mock_yield),
        ...
        call = mock_yield.call_args
        assert call.args[1] == "t_test_task_123"
        assert call.kwargs["expected_run_id"] == 7
