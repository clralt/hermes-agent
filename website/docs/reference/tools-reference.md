...
| `kanban_complete` | Mark an ordinary task done with a structured handoff. Governed cards marked `requires_independent_audit` reject this path and require trusted exact-tree close authority. | `HERMES_KANBAN_TASK` or `kanban` toolset |
...
| `kanban_phase_checkpoint` | Persist the full machine-readable phase state used for automatic clean-run continuation after an iteration cap or interruption. | Dispatcher-owned `HERMES_KANBAN_TASK` run |
...
| `kanban_create` | Fan out child tasks. Supports `work_item_kind` (`fresh`, `remediation`, `audit`) and `requires_independent_audit` for governed exact-tree closing. | `HERMES_KANBAN_TASK` or `kanban` toolset |
